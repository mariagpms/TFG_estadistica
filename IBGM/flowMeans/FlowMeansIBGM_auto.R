################################################################################
#FlowMeans automático para el conjunto IBGM
################################################################################
library(flowCore)
library(flowMeans)
library(dplyr)
...
tinic<-Sys.time()
salida<- flowMeans(datos[,c(8:19,21:34, 36:47)], Standardize = FALSE)
tfin<-Sys.time()
tiempo<-tfin-tinic
...
#Resuelve con el algoritmo del húngaro
...
df<-data.frame(Algoritmo="FlowMeans_Automatico",
...
write.table(df, file = "boxplot_bgm_flowmeansauto.csv",
            sep = "\t", row.names = F)
