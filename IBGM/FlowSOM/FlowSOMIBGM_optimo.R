#################################################################################
#FlowSOM óptimo (número de clusters prefijado) para el conjunto IBGM
################################################################################
library(flowCore)
library(flowMeans)
library(dplyr)
...
tinic<-Sys.time()
salida<-FlowSOM(input=datos,silent = FALSE, scale=TRUE, nClus = 60,colsToUse=c(8:19,21:34,36:47))
tfin<-Sys.time()
tiempo<-tfin-tinic
etiquetas_ibgm<-data.frame(Manual=exprs(datos)[,"OmiqFilter"], Clustering=GetMetaclusters(salida))
...
if(n_clusters<n_cells){
  ...
  resultados["Hungaro","Accuracy"]<-mean(etiquetas_ibgm$Clustering==asignacion_inversa[match(etiquetas_ibgm$Manual, unique(etiquetas_ibgm$Manual))])
  ...
  resultados["Hungaro","Accuracy"]<-mean(etiquetas_ibgm$Clustering==asignacion[match(etiquetas_ibgm$Manual, unique(etiquetas_ibgm$Manual))])
  resultados["Hungaro","Pr"]<-resultados["Hungaro","Pr"]/n_cells
  resultados["Hungaro","Re"]<-resultados["Hungaro","Re"]/n_cells
  resultados["Hungaro","F"]<-resultados["Hungaro","F"]/n_cells
  resultados["Hungaro","NCelulas"]<-n_cells
...
resultados["Maximo","Accuracy"]<-mean(etiquetas_ibgm$Clustering==asignacion_maximo[match(etiquetas_ibgm$Manual, unique(etiquetas_ibgm$Manual))])
...
df<-data.frame(Algoritmo="FlowSOM_Optimo",
               Asignacion=c(rep("Hungaro",100),rep("Maximo",100)),
               F_score=c(boot_etiquetas$t[,3],boot_etiquetas$t[,8]),
               Accuracy=c(boot_etiquetas$t[,5],boot_etiquetas$t[,10]),
               Pr=c(boot_etiquetas$t[,1],boot_etiquetas$t[,6]),
               Re=c(boot_etiquetas$t[,2],boot_etiquetas$t[,7]))
write.table(df, file = "boxplot_ibgm_flowsomopt.csv",
            sep = "\t", row.names = F)
