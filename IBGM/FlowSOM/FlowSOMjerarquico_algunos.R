#################################################################################
#FlowSOM óptimo (número de clusters prefijado) para el conjunto IBGM, jerárquico con solo algunos marcadores
################################################################################
library(flowCore)
library(flowMeans)
library(dplyr)
...
equivalencias <- read_csv("E:/equivalencias.csv")
...
salida<-FlowSOM(input=datos,silent = FALSE, scale = TRUE,nClus = 3,colsToUse=c(14,40))
...
unique(etiquetas_ibgm$Manual)[unique(etiquetas_ibgm$Manual)!="Otro"]
...
##########################################################################################################################
#Para las T cells
datos_tcells<-Subset(datos,etiquetas_ibgm$Clustering==3)
salida_tcells<-FlowSOM(datos_tcells,silent = FALSE, scale=TRUE, nClus = 4, colsToUse=c(14,40,9,21))
...
unique(etiquetas_tcells$Manual)[unique(etiquetas_tcells$Manual)!="Otro"]
...
salida_realtcells<-FlowSOM(datos_realtcells,silent = FALSE, scale=TRUE, nClus = 5, colsToUse=c(14,40,9,21,22,10))
...
##########################################################################################
#Para las CD8
datos_cd8<-Subset(datos_realtcells, etiquetas_realtcells$Clustering==3)
salida_cd8<-FlowSOM(datos_cd8,silent = FALSE, scale=TRUE, nClus = 7, colsToUse=c(18,8,11))
...
salida_cd4<-FlowSOM(datos_cd4,silent = FALSE, scale=TRUE, nClus = 15, colsToUse=c(18,8,11,23,44))
...
####################################################################################
#para las NKT
datos_NKT<-Subset(datos_tcells,etiquetas_tcells$Clustering==2)
salida_NKT<-FlowSOM(datos_NKT,silent = FALSE, scale=TRUE, nClus = 6, colsToUse=c(36,10))
...
####################################################################################
#para las Tgd
datos_Tgd<-Subset(datos_tcells,etiquetas_tcells$Clustering==3)
salida_Tgd<-FlowSOM(datos_Tgd,silent = FALSE, scale=TRUE, nClus = 7, colsToUse=c(18,8,11))
...
salida_bcells<-FlowSOM(datos_bcells,silent = FALSE, scale=TRUE, nClus = 4, colsToUse=c(14,40,32))
...
######################################################################################
#Para las IgD-
datos_igd<-Subset(datos_bcells,etiquetas_bcells$Clustering==2)
salida_igd<-FlowSOM(datos_igd,silent = FALSE, scale=TRUE, nClus = 4, colsToUse=c(15,28))
...
salida_plasmablasts<-FlowSOM(datos_plasmablasts,silent = FALSE, scale=TRUE, nClus = 6, colsToUse=c(26,19,16,15))
...
salida_memoryBcells<-FlowSOM(datos_memoryBcells,silent = FALSE, scale=TRUE, nClus = 6, colsToUse=c(26,19,16,15))
...
salida_nono<-FlowSOM(datos_nono,silent = FALSE, scale=TRUE, nClus = 6 , colsToUse=c(12,25,9,38,27,36))
...
#############################################################################################################
#Para las DR-
datos_dr<-Subset(datos_nono,etiquetas_nono$Clustering==1)
salida_dr<-FlowSOM(datos_dr,silent = FALSE, scale=TRUE, nClus = 6 , colsToUse=c(9,41))
...
#############################################################################################################
#Para las APC
datos_apc<-Subset(datos_nono,etiquetas_nono$Clustering==1)
salida_apc<-FlowSOM(datos_apc,silent = FALSE, scale=TRUE, nClus =6 , colsToUse=c(25,13,38))
