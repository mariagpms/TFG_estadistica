################################################################################
#FlowSOM óptimo para datos del FlowCAP-I
################################################################################
...
for (archivo in ficheros) {
  ...
  for (j in 1:length(datos)) {
    cat("\nMuestra",j,"\n")
    tinic<-Sys.time()
    salida<-FlowSOM(input=datos[[j]],silent = FALSE, scale=TRUE,nClus = numeroClus[[archivo]])
    tfin<-Sys.time()
    tiempos[[archivo]][j]<-tfin-tinic
    cat(tiempos[[archivo]][j],"\n")
    temp<-data.frame(Muestra=rep(j,length(GetMetaclusters(salida))), Clustering=as.numeric(GetMetaclusters(salida)), Manual=manual[[j]])
