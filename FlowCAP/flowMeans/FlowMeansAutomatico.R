################################################################################
#FlowMeans automático para los conjuntos del FlowCAP-I
################################################################################
...
library(dplyr) #libreria para tratar con los datos
library(boot) #libreria para boostrap
library(clue) #libreria que incluye el algoritmo del hungaro
...
for (archivo in ficheros) {
  ...
  for (j in 1:length(datos)) {
    cat("Muestra",j,"\n")
    #Se saltan las muestras de ND que dan error de singularidad
    if((archivo=="FlowCAP_ND.fcs")&(j %in% c(2,9,25))) next
    tiempos[[archivo]][j] <- system.time({
      salida<- flowMeans(datos[[j]], Standardize = FALSE)
    })
...
evaluando<-function(archivo, dataset){
  ...
  for(m in 1:n_muestras){
    ...
    #Se calcula la matriz de F_score, precision y recall
...
######################################################################################
#IC con cheap boostrap
metricas_boot<-list(
  FlowCAP_ND.fcs = NULL,
  FlowCAP_WNV.fcs = NULL)
n_boot<-10

#Devuelve las metricas para cada uno de las replicas de bootstrap
funcion_metricas<-function(temp, indices){
  ...
  #Crear matriz de f_score, precision y recall
  for (i in 1:n_clusters) {
    for (j in 1:n_cells) {
      tp<-sum(temp[,"Clustering"]==i & temp[,"Manual"]==j)
      fp<-sum(temp[,"Clustering"]==i & temp[,"Manual"]!=j)
  ...
  #Resolucion porblema de asignacion del húngaro
  if(n_clusters<n_cells){
    #Las poblaciones sin cluster se les asignará NA y tendrán valor 0 para pr, re y F
    asignacion<-solve_LSAP(F_mat, maximum = TRUE)
    asignacion_inversa<-rep(0,n_cells)
...
df_nd<-data.frame(Tipo=rep("ND_FlowMeansAuto",2*n_boot),
...
df_wnv<-data.frame(Tipo=rep("WNV_FlowMeansAuto",2*n_boot),
                  Asignacion=rep(c("Maximo","Hungaro"),each=n_boot),
...
write.table(df, file = "boxplot_flowcap_flowMeansAuto.csv",
            sep = "\t", row.names = F)
